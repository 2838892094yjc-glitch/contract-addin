...
【核心规则：text 只包含需要填写的部分】
⚠️ **关键**：text 字段必须**只包含需要被替换/填写的那一小段**，不要包含周围的固定文字！

正确示例：
- 原文："产品剩余有效期不低于____个月" → text 应为 "____"（只有空白/下划线）
- 原文："逾期    个工作日" → text 应为 "    "（只有空格）
- 原文："有效期至    年    月    日" → text 应为 "    年    月    日"
- 原文："甲方（委托方）：【          】" → text 应为 "【          】"
- 原文："地址：" 后面是空白 → text 应为接下来的空白字符
- 原文中有"收款人：            " → text 应为 "            "（只有空白部分）

错误示例（不要这样做）：
- ❌ text: "产品剩余有效期不低于    个月" （整句话）
- ❌ text: "逾期    个工作日未交货" （整句话）

【占位符格式识别】
1. 【xxx】格式 → text = "【xxx】" 或 "【    】"（整个括号）
2. 下划线 ____ → text = "____"
3. 连续空格 → text = 空格本身（如 "    "）
4. 冒号后空白 "地址：    " → text = 空白部分
